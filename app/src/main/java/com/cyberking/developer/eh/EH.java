...
public class EH implements Thread.UncaughtExceptionHandler, Application.ActivityLifecycleCallbacks {
  ...
  private static final String SHARED_PREFS_FILE = "com.cyberking.developer.handler.ehprefs";
  ...
  private void installEH () {
    try {
      if (eApplication == null)
        Log.e (EH_TAG, "EH NOT installed! NULL context.");
      else {
        eOldHandler = Thread.getDefaultUncaughtExceptionHandler ();

        if (eOldHandler instanceof EH) {
          Log.i (EH_TAG, "EH already installed!, re-installing...");
          eOldHandler = null;
        }
        else
          Log.i (EH_TAG, "EH installing... [ Replacing DefaultUncaughtExceptionHandler, called if EA is disabled. ]");

        eApplication.registerActivityLifecycleCallbacks (this);

        Thread.setDefaultUncaughtExceptionHandler (this);

        Log.i (EH_TAG, "EH installed successfuly!");
      }
    }
    catch (Throwable t) {
      Log.e (EH_TAG, "EH NOT installed.", t);
    }
  }
  ...
  public void uncaughtException (Thread thread, Throwable t) {
    if (eEnabled) {
      Log.i (EH_TAG, "EH recieved uncaught exception.", t);

      if (hasJustCrashed ()) {
        Log.e (EH_TAG, "Application just crashed. EH NOT launching to avoid restart loop...");
        ...
        if (!eIsInBackground || eRunInBackground)
          try {
            ...
            Log.i (EH_TAG, "EH started successfully!");
          }
          catch (Throwable th) {
          ...
          Log.i (EH_TAG, "EH background mode disabled!");
  ...
  private boolean launchDUEH (Thread thread, Throwable th) {
    if (eOldHandler != null) {
      Log.i (EH_TAG, "EH Launching DefaultUncaughtExceptionHandler.");
      eOldHandler.uncaughtException (thread, th);

      return true;
    }
    else
      Log.i (EH_TAG, "No DefaultUncaughtExceptionHandler present!");
